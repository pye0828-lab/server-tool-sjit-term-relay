...
소스를 내려받아 수정하려면 SSH 인증 설정 후 `clone`합니다.

> [!NOTE]
> 아래 명령어는 **Linux**와 **Windows Git Bash** 모두 동일하게 적용됩니다.  
> Git Bash는 Windows에서도 bash 환경을 제공하므로 별도 구분 없이 같은 명령어를 사용합니다.
...
| **Windows** | [Git for Windows](https://git-scm.com/download/win) 설치 후 **Git Bash** 사용 |

---

### 1. SSH 키 생성

**Git Bash(Windows)** 또는 **Linux 터미널** 에서 실행합니다.

```bash
ssh-keygen -t ed25519 -C "your-label-github" -f ~/.ssh/id_ed25519_github
```

> - `-C` : 키를 구분하기 위한 레이블 (자유롭게 설정)
> - `-f` : 키 파일 경로 및 이름
...
| 파일 | 설명 |
|------|------|
| `~/.ssh/id_ed25519_github` | 개인키 (절대 외부 유출 금지) |
| `~/.ssh/id_ed25519_github.pub` | 공개키 (GitHub에 등록) |

---

### 2. `~/.ssh/config` 설정

`~/.ssh/config` 파일을 열어 아래 내용을 추가합니다.
...
3. **New SSH key** 클릭
4. Title: 키를 구분할 이름 입력 (예: `my-laptop`, `rpi4`)
...
```
Hi pye0828-lab! You've successfully authenticated, but GitHub does not provide shell access.
```
...
> [!TIP]
> 이후 수정사항은 `git add` → `git commit` → `git push` 로 반영합니다.
